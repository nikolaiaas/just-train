...
test("the admin worker expands through separate skill rollout paths", async () => {
  const source = await readFunctionSource("process-admin-ai-job");
  ...
  assert.match(source, /claim_admin_skill_curriculum_job_for_worker/);
  assert.match(source, /complete_admin_skill_curriculum_job_for_worker/);
  assert.match(source, /admin_skill_suggestions/);
  assert.match(source, /admin_skill_package/);
  assert.match(source, /admin_skill_curriculum/);
  assert.match(source, /"content\.skill_suggestions"/);
  assert.match(source, /"content\.skill_package"/);
  assert.match(source, /"content\.skill_curriculum"/);
});
