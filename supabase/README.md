...
there, start Local Supabase, and serve both configured AI Functions from the
repository root:
...
Keep this attached process running while testing either the family image flow
or the administration assistant. The tracked Codex project action **Start
...
allowlist, and both hosted templates were aligned with the same Danish code and
scanner-safe magic-link content on 2026-08-23. Custom SMTP, disabled link
tracking, and a final synthetic or adult delivery test remain separate hosted
configuration gates; local `config.toml` settings do not change a hosted
