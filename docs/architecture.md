...
The native Supabase GitHub integration is enabled, restricted to `nikolaiaas/just-train`, and configured with working directory `.`, `main` as its production branch, and production deploys on. Automatic preview branches remain off until their paid-plan setup is reviewed. `main` is protected by current pull requests, required GitHub quality, database, and Vercel preview checks, resolved conversations, squash-only linear history, and blocked force pushes/deletion. The first end-to-end merge verification remains pending, so automatic hosted deployment is not yet an active guarantee.
